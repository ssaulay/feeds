...
Un quatrième apport, sous forme de liste de 37 erreurs courantes, prend du recul et cadre l'IA comme un problème horizontal pluriannuel plutôt qu'un projet IT ponctuel. Points saillants : investir dans la fondation data avant de déployer des outils ; partir d'un vrai problème métier plutôt que du réflexe "il nous faut de l'IA" ; repenser le workflow plutôt que se contenter de l'automatiser tel quel ; donner accès aux systèmes sous-jacents (pas juste du chat) pour que l'IA agisse réellement ; mesurer une baseline avant adoption pour pouvoir construire une histoire de ROI crédible ; instaurer des evals internes continues (coût par tâche réussie) pour suivre la performance réelle des modèles ; et traiter le change management comme aussi critique que la technologie. L'auteur insiste enfin sur le fait qu'aucun projet IA ne doit reposer sur une seule fonction ou personne : il faut un sponsor exécutif senior clair.

Les quatre apports convergent partiellement : l'idée que l'IA doit "agir" sur les systèmes sous-jacents plutôt que rester un simple chat fait écho direct au repo Team OS (accès direct aux SQL, dashboards, schémas). Le "last mile" (gouvernance, observabilité, contexte, process, personnes) recoupe aussi l'intuition que la vraie leverage n'est pas individuelle mais infrastructurelle.

⚠️ Contradiction (nuance) : l'apport sur les 37 erreurs recommande un sponsor exécutif senior clair et refuse qu'un projet IA repose sur une seule fonction/personne — ce qui semble tendre contre l'approche "Sasha" (une personne dédiée porte la transformation) et contre l'adoption organique bottom-up célébrée dans les apports 2 et 3. Les deux logiques ne sont pas nécessairement incompatibles (un sponsor exec pourrait légitimer et sécuriser un rôle terrain type Sasha ou un repo type Team OS), mais la note souligne une tension entre "sortir du mandat top-down flou" et "avoir un portage exécutif clair" comme condition de succès.
...
⚠️ Contradiction : cette perspective interroge en creux les logiques valorisées ailleurs dans la note — leverage IA, automatisation cumulative, agentivité de niveau 5, productivité d'équipe — qui reposent précisément sur des métriques immédiates et visibles (temps gagné, tâches automatisées, dashboards, promotions). L'apport sur les 37 erreurs renforce d'ailleurs cette logique de mesure court-termiste en insistant sur la nécessité d'une baseline et d'un ROI démontrable. Christensen suggère que ces récompenses court-termistes, aussi légitimes soient-elles au travail, peuvent masquer un déséquilibre de fond si elles deviennent le seul système de mesure d'une vie réussie, y compris pour ceux qui pilotent ces transformations organisationnelles.

## Sources

- [37 erreurs courantes dans la transformation IA des entreprises](../sources/2026/2102763567422251107-37-erreurs-transformation-ia.md)
