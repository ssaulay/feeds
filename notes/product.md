...
L'approche d'Aesty (suppression pure et simple de la friction d'onboarding, traitement on-device pour préserver la confidentialité) illustre un contre-exemple concret et positif à ce rant : plutôt que d'ajouter des couches (authentification, abonnements, IA imposée) qui dégradent l'expérience, l'équipe retire une étape entière du parcours utilisateur et choisit une architecture qui renforce explicitement la confiance sur les données — démontrant que réduction de friction et respect de la confidentialité peuvent être traités comme des leviers produit à part entière, et non des à-côtés.

⚠️ Contradiction : la culture de shipping rapide décrite par Cat Wu (Anthropic) valorise les mises à jour fréquentes, les produits « à la limite du fonctionnel » (voire non fonctionnels, en pari sur le futur) et l'itération continue comme moteurs de progrès produit. Le rant satirique dénonce au contraire les mises à jour forcées et les produits imposés comme sources de dégradation de l'expérience utilisateur. Ces deux perspectives illustrent une tension non résolue entre vitesse d'innovation côté producteur et stabilité/confort attendus côté utilisateur final. La méthode des AI evals de Teresa Torres offre un début de réponse à cette tension : en exigeant une mesure rigoureuse du taux de réussite avant de considérer une fonctionnalité IA comme exploitable, elle introduit un garde-fou qui pourrait limiter les dérives dénoncées par le rant tout en préservant la vitesse d'itération. Le cas Aesty confirme qu'un tel garde-fou est atteignable en pratique : identifier puis éliminer la friction réelle (plutôt que l'optimiser superficiellement) permet de concilier vitesse d'exécution produit et confort utilisateur.
