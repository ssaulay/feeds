...
Sur le plan des architectures et méthodes d'entraînement, **Diogo Almeida** (ex-OpenAI, co-inventeur de ChatGPT) présente **Jev** dans un tech talk de 36 minutes, un modèle frontier développé après deux ans en stealth mode. Jev se positionne comme une « intelligence composable » optimisée pour la prise de décision plutôt que pour la conversation, en rupture explicite avec les outils d'assistance actuels nécessitant un humain dans la boucle (Claude Code, Codex). L'angle central de ce tech talk est une remise en cause du **RLHF** comme paradigme dominant : Almeida évoque une méthode d'entraînement alternative (nommée RLCD dans une annonce antérieure) pour ouvrir une ère « post-RLHF » sans supervision humaine. Sa formule résume le constat de départ : l'IA actuelle serait « trop belle pour être vraie, trop mauvaise pour être utile ». Les chiffres annoncés restent spectaculaires — 20 à 200x plus rapide et 40 à 400x moins cher que les modèles actuels, tokens de sortie gratuits, et désormais une promesse de **zéro hallucination** — avec en prime l'incitation à devenir « Jev Engineer », suggérant l'émergence d'un nouveau rôle/métier autour de cette technologie.

⚠️ Contradiction (mineure, sur la précision des chiffres) : une source cite une plage « 20 à 200x plus rapide / 40 à 400x moins cher », une autre fixe des valeurs uniques « 200x plus rapide / 400x moins cher » — signe que les chiffres communiqués varient selon la présentation et n'ont pas de valeur de référence stabilisée.

Ces claims (vitesse, coût, zéro hallucination, fin du RLHF) restent pour l'instant des annonces non étayées par des benchmarks indépendants ni de détails techniques publics sur la méthode d'entraînement : à rapprocher directement du constat critique ci-dessous sur la fiabilité des métriques et scores annoncés, qui invite à suspendre le jugement tant qu'un examen qualitatif indépendant n'est pas disponible — d'autant plus qu'une promesse de « zéro hallucination » est en soi une affirmation extraordinaire pour un LLM.

Enfin, un angle plus critique porte sur l'**évaluation des LLMs** : les scores pass/fail des benchmarks sont jugés trompeurs. De nombreux échecs proviennent en réalité de critères de test trop rigides ou de tests cachés mal calibrés, plutôt que de véritables erreurs du modèle — une réponse peut être plus pertinente que celle attendue par l'évaluation tout en étant comptée comme un échec. Ce constat invite à examiner les réponses individuelles derrière les chiffres agrégés plutôt qu'à se fier aux scores bruts pour juger de la qualité réelle d'un modèle. Ce point questionne indirectement la solidité des métriques mises en avant ailleurs dans cette note (par exemple le score MTEB de Gemini Embedding 2, ou les gains de vitesse/coût/hallucination annoncés par Jev) : un score chiffré, même impressionnant, ne dispense pas d'un examen qualitatif des cas d'usage réels.

## Sources

- [JEV : la promesse d'une ère post-RLHF pour les LLM sans humain dans la boucle](../sources/2026/2101294219633529030-jev-post-rlhf-llm-era.md)
- [Lancement de Jev : un modèle IA frontier optimisé pour les décisions via RLCD](../sources/2026/2099925682726002904-jev-nouveau-modele-ia-rlcd.md)
